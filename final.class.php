 ...
 */
...

        {
                if ($queryType !== "detail" && $queryType !== "news") {
                        $retData["status"]=1;
                        $retData["message"]="'$queryType' isnt 'detail' or 'news'";
                } else {
                        try {
                                ...
                                $retData["status"]=0;
                                $retData["message"]="insertion of '$stockTicker' for: '$queryType' and '$jsonData' accepted";
                        }
                        catch (Exception $e) {
                                $retData["status"]=1;
                                $retData["message"]=$e->getMessage();
...

        {
                if (is_numeric($date)) {
                        $retData["status"]=1;
                        $retData["message"]="'$date' isnt of format 'YYYY-MM-DD'";
                } else {
                        try {
                                $retData["result"] = GET_SQL("select * from stock where dateTime like ? order by dateTime", $date . "%");
                                $retData["status"]=0;
                                $retData["message"]="GET for: '$date' was accepted";
                        }
                        catch (Exception $e) {
                                $retData["status"]=1;
                                $retData["message"]=$e->getMessage();
